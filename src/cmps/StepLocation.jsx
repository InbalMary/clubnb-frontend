            ...
            (pos) => {
                ...
                if (geocoderRef.current) {
                    geocoderRef.current.geocode({ location: { lat, lng } }, (results, status) => {
                        setAddress(status === "OK" && results?.[0]?.formatted_address ? results[0].formatted_address : "Current Location")
                    })
                } else {
                    ...
                    {showDropdown && (
                        <div className="location-dropdown">
                            <button className="location-dropdown-item current-location" type="button" onClick={handleCurrentLocation}>
                                <span className="location-icon"><img src={`/img/step3/map-address.svg`} alt="map-address" className="step4-icon" />Use my current location</span>
                            </button>
